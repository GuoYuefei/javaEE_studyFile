...
public class MyRecode {
	...
	public static void main(String[] args) {
		MyRecode test = new MyRecode("wifjetfoil等长度服多少分ab", "sdfv的撒的");
		try {
...
//		System.out.println(Integer.toBinaryString(-1<<3&0xff));
	...
	/**
	 * 将word（）和octal（）的值一同输出，中间无标记
	 * @return
	 */
	public String reciding(){
		try {
			return word()+octal();
		} catch (UnsupportedEncodingException e) {
			e.printStackTrace();
...
//			System.out.println(b&0x38>>>3);			//移位运算优先级在&之前 故采用先移位后&
	...
	/**
	 * 该构造自动创建随机密匙key，该key的元素只含26个字母,长度为16字符
	 * @param word	要加密的文本
	 */
	public MyRecode(String word) {
		char[] chars={'Q','W','E','R','T','Y','U','P','A','S','D','F','G','H','J','K','L','Z','X',
				'C','V','B','N','M','O','I','q','w','e','r','t','y','u','i','o','p','a','s','d','f',
