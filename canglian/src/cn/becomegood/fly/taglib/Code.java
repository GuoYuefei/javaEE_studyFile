package cn.becomegood.fly.taglib;
...
public class Code extends HttpServlet implements Tag{
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getSession().setMaxInactiveInterval(600);		//有效时间是十分钟
		//创建一张彩色图片
...
//		out.close();
	...
	public int doEndTag() throws JspException {
		...
		String src = context+"/code?var="+var;
		//该输出字符串蕴含了javascrpt的代码，比较复杂
		String outString ="<div onclick='this.innerHTML=\"<img src="
						+ src+"&r=\"+Math.random()+\"></img>\"'>"
						+"<img alt=\"验证码\" src=\""
						+ src+"\" />"
						+"</div>";
		try {
			//通过参数传递var值，那是因为继承的接口和类在实现时各自实例化成两个不同的对象
//			out.print("<img alt='验证码' src='"+src+"'/>");
			out.print(outString);
		} catch (IOException e) {
			e.printStackTrace();
		}
